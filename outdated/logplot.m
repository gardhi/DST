...
min_PV = 10;               % Min PV power simulated [kW]
max_PV = 300;               % Max PV power simulated [kW]
step_PV = 10;                % PV power simulation step [kW]
min_batt = 10;             % Min Battery capacity simulated [kWh]
max_batt = 1400;            % Max Battery capacity simulated [kWh]
...
for year = loadCurve_titles                                   % outer loop going through all the different data sets
    ...
    path_to_dataBase = get_path('database\');
    ...
    flow_from_batt_every_scenario = zeros(length(irr), n_PV, n_batt);
    ...
    nomAmbientTemp = 20;                 % Nominal ambient test-temperature of the panels [C] % todo in fullYear script this was 25 and in SAPV it was 20. which one?
    nomCellTemp = 47;                 % Nominal Operating Cell Temperature [C]
    ...
    Den_rainflow = zeros(n_PV, n_batt);
    
    for PV_i = 1 : n_PV                                                 
        PVpower_i = min_PV + (PV_i - 1) * step_PV;                      % iteration on PV power
        T_cell = T_amb + irr .* (nomCellTemp - nomAmbientTemp) / irr_nom;              % Cell temperature as function of ambient temperature [C]
        eff_cell = 1 - coeff_T_pow .* (T_cell - nomAmbientTemp);                 % cell efficiency as function of temperature
        P_pv = irr .* PVpower_i .* eff_cell .* BoS;                 % array with Energy from the PV ( ) for each time step throughout the year. see p.191 of thesis Stefano Mandelli
        ...
        for batt_i = 1 : n_batt                                               
            ...
            Pow_max = batt_ratio * batt_cap_i;                          % maximum power acceptable by the battery
                                                      % counter for number of cycles battery goes through. Needed for CyclesToFailure()
            ...
            for t = 1 : size(Load,2)                                    
                if t > 8 
                    if batt_balance(t-1) > 0 && batt_balance(t-2) > 0 && batt_balance(t-3) > 0 && batt_balance(t-4) > 0 && batt_balance(t-5) > 0 && batt_balance(t-6) > 0 && batt_balance(t-7) > 0 && batt_balance(t-8) > 0 && batt_balance(t) < 0   % battery has been charged the previous 8 hours but not this hour.
                       ...
                       cycles_failure = cycles_to_failure(DoD);
                       Den_rainflow(PV_i, batt_i) = Den_rainflow(PV_i,batt_i) + 1/(cycles_failure);
                    end
                end
...
%             
%             if  batt_cap_i == 770 && PVpower_i == 170     %PV_i == 2    % temporary bad solution
...
%             end
            ...
            years_to_go_batt = 1/Den_rainflow(PV_i,batt_i);                                          % batteries should be replaced after this number of years
...
% budget = 1300000;                                                % budget constraint [?]
% counter_fill = 0;
% counter_empty = 0;
% for i = 1:n_PV
%     for j = 1:n_batt
